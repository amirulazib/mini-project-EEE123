...
int loadFoodData(FoodItem foodList[]) {
    ...
    int count = 0;
    while (count < ITEMS && !(file.eof())) {
        getline(file, foodList[count].code);
        getline(file, foodList[count].name);
        file >> foodList[count].price;
        file.ignore();
        count++;
    }
...
void displayFood(const FoodItem foodList[], int count) {
    ...
    for (int i = 0; i < count; i++) {
        cout << left << setw(5) << (i + 1) << setw(20) << foodList[i].code << setw(50) << foodList[i].name << "RM " << fixed << setprecision(2) << setw(30) << foodList[i].price << endl;
    }
}
